...
def fourier_prep(
    ...
    if grad_method == GradMethod.spectral:
        ...
        dudy = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[1] * u_hat)))
        dudz = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[0] * u_hat)))
        dvdx = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[2] * v_hat)))
        dvdy = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[1] * v_hat)))
        dvdz = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[0] * v_hat)))
        dwdx = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[2] * w_hat)))
        dwdy = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[1] * w_hat)))
        dwdz = np.real(fft.ifftn(fft.ifftshift(1j * k_mesh[0] * w_hat)))
